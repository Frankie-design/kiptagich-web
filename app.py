...
# ==========================================
# 1. PRESENTATION DATABASE / REGISTRY
# ==========================================
FARM_REGISTRY = {
    "11405938": {
        "owner": "David Kiprono",
...
# Explicit spatial boundary polygon coordinates for Kiptagich Ward
WARD_BOUNDARY = [
    [-0.5350, 35.5500],
    [-0.5320, 35.5700],
    [-0.5400, 35.5850],
...
def index():
    ...
    # Base interactive map centered on Kiptagich Ward area
    m = folium.Map(
    ...
    # Render foundational Kiptagich Ward perimeter line
    folium.Polygon(
    ...
    # Render static registry location markers
    ...
    if search_id:
        if farm_data:
            # Re-center viewport smoothly onto searched target location coordinates
            m = folium.Map(
            ...
            # Maintain ward polygon continuity on active viewports
            folium.Polygon(
                locations=WARD_BOUNDARY,
                color="purple",
                weight=3,
            ...
            # Highlight chosen farm layout with custom icons and geometric buffers
            folium.Marker(
                location=[farm_data["lat"], farm_data["lon"]],
            ...
            if farm_data["status"] == "Critical":
                print(f"--- SIMULATION LOG: Critical threshold breached for {farm_data['owner']}. SMS workflow successfully logged. ---")
        else:
