...
# Agrometeorological Framework Data Streams (CYGNSS, Sentinel-2, ERA5, CHIRPS, Open-Meteo)
LATEST_METRICS = {
    "soil_moisture": 23.8,   # CYGNSS 6h Temporal Baseline
    "ndvi": 0.55,            # Sentinel-2 Crop Vitality Index
    "era5_et": 4.1,          # ERA5-Land Hourly Evapotranspiration
    "rain": 0.0,             # CHIRPS Daily Rainfall Accumulation
    "et0": 3.2,              # Open-Meteo Reference ET0 Forecast
    "soil_texture": "Loam"   # KENSOTER Regional Baseline Profile
}

def fetch_and_analyze_daily_data():
    """Triggers back-end calls to pull down current Open-Meteo telemetry."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Map values into live telemetry slots
            ...
            # Calculate Dynamic NDVI based on current moisture profiles
...
def wgs84_to_utm36s(lon, lat):
    """Calculates UTM Zone 36S projected coordinate conversions from WGS84 Geodetic base."""
    lon_origin = 33.0
...
def process_csv_rows(file_stream, append_to_file=False):
    ...
    # Open local file for permanent storage backing if requested
...
def index():
    ...
    for fid, f in FARM_REGISTRY.items():
        ...
        popup_html = (
            ...
            f"<b>💧 CHIRPS Rainfall:</b> {LATEST_METRICS['rain']:.1f} mm<br>"
...
def upload_csv():
    """Accepts, decodes, and routes the submission down to storage frameworks."""
