...
# Global In-Memory System Database for True Scalability
FARM_REGISTRY = {}
IS_INITIALIZED = False
LATEST_METRICS = {"soil_moisture": 23.8, "et0": 3.2, "ndvi": 0.55, "rainfall": 0.0}

# ==========================================
# AUTOMATED AGROMET DATA ENGINE
# ==========================================
def fetch_and_analyze_daily_data():
    """
    Runs automatically to query live open-meteo satellite layers.
    Simulates the scheduled daily 6:00 PM telemetry analysis loop.
    """
    global LATEST_METRICS, FARM_REGISTRY
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Map raw data points to real project parameters
            ...
            # Trigger Automated SMS dispatch routine if any farm hits critical levels
            trigger_automated_sms_dispatch()
            
    except Exception as e:
        print(f"Background Sync Error: {e}")

def trigger_automated_sms_dispatch():
    """Loops through active plots to instantly dispatch alerts if criteria are met."""
    global FARM_REGISTRY, LATEST_METRICS
    # This routine monitors the database. We will insert the gateway codes here in Step 3!
    pass
                ...
                if wkt:
                    ...
                    if lon and lat:
                        ...
                        FARM_REGISTRY[unique_id] = {
                            ...
                            "lat": lat, "lon": lon, "utm_x": ex, "utm_y": ny, "boundary": geom
                        }
        IS_INITIALIZED = True
        # Fire initial telemetry query immediately upon system initialization
...
def index():
    ...
    # Evaluate thresholds globally for all polygons based on latest data cache
    # Raised threshold to 26.0 intentionally to catch live values for testing demo
    ...
    for fid, f in FARM_REGISTRY.items():
        if f["boundary"]:
            folium.Polygon(locations=f["boundary"], color=status_color, weight
